	...
	float3 position, float3 direction, float3 right, float3 up, float frame_weight)
{
		for (int x = 0; x < width; x++)
		{
			...
			for (int y = 0; y < height; y++)
			{
				...
				//	ray ray3(position, ray_direction + u_delta * right - v_delta
				//* up); 	payload payload3 = trace_ray(ray3, 1);
				//
				//	cg::color accumed_color{
				//		(payload0.color.r + payload1.color.r + payload2.color.r +
				//payload3.color.r) / 4.f, 		(payload0.color.g + payload1.color.g
				//+ payload2.color.g + payload3.color.g) / 4.f, 		(payload0.color.b
				//+ payload1.color.b + payload2.color.b + payload3.color.b)
				/// 4.f  }; 	render_target->item(x, y) =
				//RT::from_color(accumed_color);
				//}
				...
				cg::color accumed_color{ (accumed.r * ifw + payload.color.r * fw),
	...
	raytracer<VB, RT>::trace_ray(const ray& ray, size_t depth, float max_t, float min_t) const
{
	...
	if (closest_hit_payload.t < max_t)
	{
		if (closest_hit_shader)
			return closest_hit_shader(ray, closest_hit_payload, *closest_triangle, depth - 1);
	}
	return miss_shader(ray);
}
